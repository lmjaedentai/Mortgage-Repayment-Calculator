...
def next():
    ...
    def check(data):
        ...
        try:
            data = float(data)
            if data <1:
                messagebox.showwarning('No negative input !','Please input a positive value')
                return
        except ValueError:
            messagebox.showwarning('Incorrect Input','Please input a correct value!\nAll input should be in decimals only')
        else:
            proceed = proceed + 1         
        if proceed > 3:
            calculate()
...
def files(): #export xlsx
	global totalperiod,totalinterest,totalprinciple,monthpay
	import csv
	import os
	...
	if os.path.exists(os.path.join(thepath,'mortage loan.csv')):  #error handling
		try:
			...
			return
		print('deleted')
	messagebox.showinfo('Done',"Your CSV file was exported to "+str(thepath)+"\nRight click and open it by Excel or other suitable program")
...
#GUI
root = Tk()
root.title (" House Loan Calculator")
